...
\begin{figure}
  \begin{center}
    \includegraphics[width=0.8 \linewidth]{figure/retargeting-result1.jpg}
    \caption{HRP-4C optimized motion using RobOptim and the Choreonoid
      robotics frameworks (yellow is the final result, white the
      original marker set).}
...
recorded from a human subject using a motion capture setup.


The Ipopt plug-in for RobOptim allowed us to solve the problem while
using sparse matrices computation. This proved particularly useful for
trajectroy optimization where large matrices are involved. However
...
Fig.~\ref{fig:retargeting} (post-processing) and
